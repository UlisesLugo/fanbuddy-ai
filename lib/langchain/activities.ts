...
const ActivityItemSchema = z.object({
  ...
  recommendedTime: z
    .string()
    .describe(
      'Time of day for this activity, e.g. "09:00–11:00" or "Evening (19:00)"',
    ),
  tip: z.string().optional(),
});

const DayActivitiesSchema = z.object({
  day: z.enum(['arrival', 'match', 'departure', 'free']),
  date: z.string(),
  label: z.string(),
...
export interface DayEntry {
  day: 'arrival' | 'match' | 'departure' | 'free';
  date: string;
...
export function buildDayEntries(
  ...
  const kickoffDate = new Date(kickoffUtc.slice(0, 10));
  const entries: DayEntry[] = [];

  for (
    let i = new Date(checkInDate);
    i <= checkOutDate;
    i.setDate(i.getDate() + 1)
  ) {
    if (i.getTime() == kickoffDate.getTime()) {
      entries.push({
        day: 'match',
        date: kickoffDate.toISOString().slice(0, 10),
        availableHours: 4,
      });
    } else if (i.getTime() == checkInDate.getTime()) {
      entries.push({ day: 'arrival', date: checkIn, availableHours: 6 });
    } else if (i.getTime() == checkOutDate.getTime()) {
...
export function buildActivitiesPrompt(
  match: Pick<
    RawMatchFixture,
    'homeTeam' | 'awayTeam' | 'kickoffUtc' | 'match_city' | 'venue'
  >,
  travelDates: { checkIn: string; checkOut: string },
): string {
  ...
  const kickoffTimeUtc = new Date(match.kickoffUtc).toLocaleTimeString(
    'en-GB',
    {
      hour: '2-digit',
      minute: '2-digit',
  ...
  const dayLines = entries
    .map((e) => {
      ...
      const matchNote =
        e.date === kickoffDate
          ? ` (match kicks off at ${kickoffTimeUtc} UTC)`
          : '';
